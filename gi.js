...
GI.prototype = {
		...
		for(var i = 0, l = this.meshs.length; i < l; i++ ){
			...
			if(index){
				...
				for(var n = 0; n < length; n++){
					...
					offset1 = Math.round(( Math.round(uv21.y * this.height + 0.5) * this.width) + uv21.x * this.width + 0.5);
					
					offset2 = Math.round(( Math.round(uv22.y * this.height + 0.5) * this.width) + uv22.x * this.width + 0.5);
					offset3 = Math.round(( Math.round(uv23.y * this.height + 0.5) * this.width) + uv23.x * this.width + 0.5);
	...
	splitTri : function(tri){
		...
		var centerPoint = new THREE.Vector3(
			(tri.position[0][0] + tri.position[1][0] + tri.position[2][0]) / 3,
			(tri.position[0][1] + tri.position[1][1] + tri.position[2][1]) / 3,
			(tri.position[0][2] + tri.position[1][2] + tri.position[2][2]) / 3,
		);

		var edge1 = new THREE.Vector3(centerPoint.x - tri.position[0][0], centerPoint.y - tri.position[0][1], centerPoint.z - tri.position[0][2]);
		var edge2 = new THREE.Vector3(centerPoint.x - tri.position[1][0], centerPoint.y - tri.position[1][1], centerPoint.z - tri.position[1][2]);
		var edge3 = new THREE.Vector3(centerPoint.x - tri.position[2][0], centerPoint.y - tri.position[2][1], centerPoint.z - tri.position[2][2]);
		...
		var centerNormal = new THREE.Vector3();
		centerNormal = centerNormal.set(
			(tri.normal[0][0] * (d1 / sum) + tri.normal[1][0] * (d2 / sum) + tri.normal[2][0] * (d3 / sum)),
			(tri.normal[0][1] * (d1 / sum) + tri.normal[1][1] * (d2 / sum) + tri.normal[2][1] * (d3 / sum)),
			(tri.normal[0][2] * (d1 / sum) + tri.normal[1][2] * (d2 / sum) + tri.normal[2][2] * (d3 / sum)),
		).normalize();


		var uv2 = new THREE.Vector2(
			(tri.uv2[0][0] * (d1 / sum) + tri.uv2[1][0] * (d2 / sum) + tri.uv2[1][0] * (d3 / sum)),
			(tri.uv2[0][1] * (d1 / sum) + tri.uv2[1][1] * (d2 / sum) + tri.uv2[1][1] * (d3 / sum)),
		);
		...
		this.triangles.push(
			{
				position : [tri.position[0], tri.position[1], [centerPoint.x, centerPoint.y, centerPoint.z]],
				normal : [tri.normal[0], tri.normal[1], [centerNormal.x, centerNormal.y, centerNormal.z]],
				uv2 : [tri.uv2[0], tri.uv2[1], [uv2.x, uv2.y]],
				offset: [tri.offset[0], tri.offset[1], offset],
				weight : edge1.clone().cross(edge2).lengthSq(),
			},
			{
				position : [tri.position[2], tri.position[1], [centerPoint.x, centerPoint.y, centerPoint.z]],
				normal : [tri.normal[2], tri.normal[1], [centerNormal.x, centerNormal.y, centerNormal.z]],
				uv2 : [tri.uv2[2], tri.uv2[1], [uv2.x, uv2.y]],
				offset: [tri.offset[2], tri.offset[1], offset],
				weight : edge3.clone().cross(edge2).lengthSq(),
			},
			{
				position : [tri.position[0], tri.position[2], [centerPoint.x, centerPoint.y, centerPoint.z]],
				normal : [tri.normal[0], tri.normal[2], [centerNormal.x, centerNormal.y, centerNormal.z]],
				uv2 : [tri.uv2[0], tri.uv2[2], [uv2.x, uv2.y]],
				offset: [tri.offset[0], tri.offset[2], offset],
	...
	propagate : function(){
		...
		for(var i = 0; i < 16; i ++){
			...
			var area = edge1.clone().cross(edge2).length();
			...
			for(var x = minX + 1, xl = maxX - minX; x < xl; x++){
				for(var y = minY + 1, yl = maxY - minY; y < yl; y++){
					var posX = minX + x;
					var posY = minY = y;
					...
					area1 = tempEdge1.cross(tempEdge2).length;
					
					tempEdge1.set(pos2.x - posX, pos2.y - posY);
					tempEdge2.set(pos3.x - posX, pos3.y - posY);

					area2 = tempEdge1.cross(tempEdge2).length;

					tempEdge1.set(pos1.x - posX, pos1.y - posY);
					tempEdge2.set(pos3.x - posX, pos3.y - posY);

					area3 = tempEdge1.cross(tempEdge2).length;

					if(area1 + area2 + area3 <= area + 0.1){
						var offset = posX * width + posX;
						if(!this.lightsData[offset]){
